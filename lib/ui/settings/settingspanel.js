...
export default class SettingsPanel extends React.Component {
  ...
  getButtonBySettingsKey() {
    if (this.state.activeKey === 0) {
      return (
        <React.Fragment>
        <span>  Layout Name: <strong>{this.props.instrumentPanel.layoutName}</strong></span>
      ...
      return (
        <span> Reset Instrument Panel settings</span>
      );
    }
    return null;
  }
  ...
  getContentBySettingsKey() {
    if (this.state.activeKey === 0) {
      ...
      return (
        <DarkModeSettings instrumentPanel={this.props.instrumentPanel}/>
      )
    } else if (this.state.activeKey === 3) {
      return (
