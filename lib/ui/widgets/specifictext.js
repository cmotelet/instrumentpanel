...
function environmentCurrent() {
  ...
  var setTrue = protectedConversion(conversions['deg'])(this.state.value.setTrue);
  var setMagnetic = protectedConversion(conversions['deg'])(this.state.value.setMagnetic);
  ...
  if (convertTo !== unit) {
    ...
    if (typeof conversions[convertTo] === 'function') {
      unit = convertTo;
      drift = protectedConversion(conversions[convertTo])(drift);
    }
  }
  return (
...
function designLength() {
  ...
  if (convertTo !== unit) {
    ...
    if (typeof conversions[convertTo] === 'function') {
      unit = convertTo;
      overall = protectedConversion(conversions[convertTo])(overall);
      hull = protectedConversion(conversions[convertTo])(hull);
      waterline = protectedConversion(conversions[convertTo])(waterline);
    }
  }
  return (
    <svg height="100%" width="100%" viewBox={this.props.viewBox}>
