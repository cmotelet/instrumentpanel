...
export default class CompassRoseComponent extends React.Component {
  ...
  renderDirName() {
    var a = safeAngle(this.state.value);
    if (a !== this.state.value) return '---';
  ...
  render() {
    const safeValue = safeAngle(this.state.value);
    const safeValueText = (safeValue !== this.state.value) ? 'NaN' : safeValue.toFixed(0);
    return (
      <svg height="100%" width="100%" viewBox="0 0 400 400">
      {this.renderGradient()}
      <g transform="scale(0.8) translate(50 50)">
        <g stroke="black" transform={centerRotate(-safeValue)}>
          {this.renderTicks()}
          {mainDirs}
          {minorDirs}
        ...
        <text x="200" y="325" textAnchor="middle" fontSize="28">{safeValueText}</text>
