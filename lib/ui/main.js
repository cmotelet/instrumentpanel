...
import {retrieveDarkMode} from '../util/localstorage';
import {darkModeSetByOs} from './settings/darkmodesettings';

var streamBundle = new StreamBundle();
var instrumentPanel = new InstrumentPanel(streamBundle);
const notificationPageId = -1;
instrumentPanel.colorScheme = prefersColorScheme();
...
class App extends React.Component {
  constructor(props) {
    ...
    console.log('css-prefers-color-scheme hasNativeSupport:', instrumentPanel.colorScheme.hasNativeSupport);
    console.log('colorScheme:', instrumentPanel.colorScheme.scheme);
    const initialDarkMode = retrieveDarkMode();
    if (initialDarkMode.darkModeSetBy !== darkModeSetByOs) {
      instrumentPanel.colorScheme.removeListener();
    } else {
        initialDarkMode.darkModeCurrent = instrumentPanel.colorScheme.scheme;
      }
    instrumentPanel.colorScheme.onChange = () => {
      console.log('new colorScheme:', instrumentPanel.colorScheme.scheme);
    };
    instrumentPanel.colorScheme.scheme = initialDarkMode.darkModeCurrent;
    instrumentPanel.darkMode = initialDarkMode;
  }
