...
// https://github.com/STRML/react-grid-layout/issues/382#issuecomment-299734450
  ...
  generateToolTipsElement(widget) {
    class ToolTipsComponent extends React.Component {
      ...
      render() {
        ...
        return React.createElement(
          'div',
          {
            ...
            style: {backgroundPositionX: - (this.props.widget.pathTimeoutState) * 40 + 'px'},
      ...
      calcLastSpeak(epochTimeStamp) {
        return (!isNaN(epochTimeStamp)) ? parseInt((Date.now() - epochTimeStamp) / 1000) + 's' : '???';
      }
      ...
      toolTipsContent(widget) {
        ...
        if (this.state.toolTipsVisible) {
          sourceIdContent = '<b>sourceId:</b><br>';
          var epochTimeStamp;
          var lastSpeak;
          var pathArray;
          widget.handledSources.forEach( (source) => {
            pathArray = source.path.split('.');
            epochTimeStamp = instrumentPanel.streamBundle.getEpochTimeStampForKey(source.key);
            lastSpeak = ' (' + this.calcLastSpeak(epochTimeStamp) + ')';
            sourceIdContent += source.sourceId + ' ... ' + pathArray[pathArray.length -1] + lastSpeak + '<br>';
          });
        }
