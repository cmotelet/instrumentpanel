...
// https://github.com/STRML/react-grid-layout/issues/382#issuecomment-299734450
    ...
    var ratioVisiblePath = function() {
      const visiblePaths = instrumentPanel.getEnabledWidgets().length;
      const hiddenPaths  = instrumentPanel.getDisabledWidgets().length;
      if (hiddenPaths > 0) {
        return (
          <div style={{'text-align':'center'}}>{visiblePaths}/{visiblePaths + hiddenPaths} visible path{(visiblePaths>1)?'s':''} on grid</div>
        )
      }
    }
