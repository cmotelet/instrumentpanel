...
export default class IpNavBar extends React.Component {
  constructor(props) {
    ...
    onSwipe({ node: document, bubbles: true});
    document.addEventListener('swipe', (event) => {
      this.swipePage(event.detail.direction);
    });
  }

  swipePage(directionText) {
    if (!directionText.match(/left|right/)) {
      return;
    }
    const direction = directionText === 'left' ? +1 : -1;
    this.props.instrumentPanel.gotoNextPrevPage(direction);
  }
  ...
  showIpNavBar(evt) {
    clearInterval(this.hideNavBarTimer);
