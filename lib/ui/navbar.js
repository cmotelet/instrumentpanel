...
import {darkModeLight, darkModeDark} from './settings/darkmodesettings';
...
export default class IpNavBar extends React.Component {
  ...
  connectedButtons() {
    ...
    return (
      <span>
        ...
        <Button bsClass="navbar-btn" className="btn pull-right navbar-btn-icon"
            onClick={this.toggleDarkMode} disabled={disableButton}>
          {darkModeIcon}
        </Button>
      </span>
    );
  ...
  toggleDarkMode() {
    var newDarkModeToSave = localstorageRetrieveDarkMode();
    ...
    this.props.instrumentPanel.setDarkModeSetByMainbar();
