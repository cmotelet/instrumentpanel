...
export default function InstrumentPanel(streamBundle) {
  ...
  var handleCheckDeadPath = setTimeout(() => {
      this.checkDeadPath()
    },
    10000
  );
...
InstrumentPanel.prototype.getActiveWidgets = function() {
  ...
  map(function(widget) {
    ...
    return {
      ...
      pathTimeoutState: widget.pathTimeoutState,
...
InstrumentPanel.prototype.checkDeadPath = function checkDeadPath() {
  var pathTimeoutStateChanged = false;
  var newPathTimeoutState;
  var tmpPathTimeoutState;
  var lastSpeak;
  var epochTimeStamp;
  var widgets = this.getEnabledWidgets(this.currentPage);
  widgets.forEach( widget => {
    newPathTimeoutState = 0;
    widget.getHandledSources().forEach( source => {
      tmpPathTimeoutState = 0;
      epochTimeStamp = this.streamBundle.getEpochTimeStampForKey(source.key);
      if (isNaN(epochTimeStamp)) {
        tmpPathTimeoutState = 3;
      } else {
          lastSpeak = parseInt((Date.now() - epochTimeStamp) / 1000);
          if (lastSpeak >= 45) {
            tmpPathTimeoutState = 2;
          } else if (lastSpeak >= 30) {
              tmpPathTimeoutState = 1;
            }
        }
      if (tmpPathTimeoutState > newPathTimeoutState) {
        newPathTimeoutState = tmpPathTimeoutState;
      }
    });
    if (widget.pathTimeoutState !== newPathTimeoutState) {
      widget.pathTimeoutState = newPathTimeoutState;
      pathTimeoutStateChanged = true;
    }
  });
  if (pathTimeoutStateChanged) {
    this.pushGridChanges();
  }
  var handleCheckDeadPath = setTimeout(() => {
      this.checkDeadPath()
