...
const widgetActiveModes = require('../ui/settings/settingspanel').widgetActiveModes;
...
InstrumentPanel.prototype.getWidgetActiveModeByPath = function(path) {
  ...
  return (pathsFilter.indexOf(path) >= 0) ? true : false;
}
...
InstrumentPanel.prototype.handlePossiblyNewSource = function(newSource) {
  ...
  if (!handled) {
    var widget = new module.constructor(handlePage + '-' + this.getWidgets(handlePage).length, {
      ...
      key: newSource.key,
      path: newSource.path,
      active: this.getWidgetActiveModeByPath(newSource.path)
...
InstrumentPanel.prototype.loadWidgetActiveMode = function loadWidgetActiveMode() {
  this.widgetActiveMode = localstorageRetrieveWidgetActiveMode();
  this.saveWidgetActiveMode();
}

InstrumentPanel.prototype.saveWidgetActiveMode = function WidgetActiveMode() {
...
InstrumentPanel.prototype.getPathsByWidgetType = function getPathsByWidgetType(widgetType) {
